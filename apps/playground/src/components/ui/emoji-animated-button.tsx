...
const AnimatedToggleButton = ({
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <button
                    ...
                    className={cn(
                        ...
                        label.text
                            ? 'gap-1 px-2 py-1 border h-8'
                            : 'gap-2 border h-8 w-8 justify-center',
                        isActive
                ...
                >
                    <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                        ...
                        >
                            <label.icon
                                className={cn('w-4 h-4', isActive ? color_?.text : 'text-inherit')}
                            />
                        </motion.div>
                    </div>
                    {label.text && (
            ...
            <PopoverContent align="end" className="w-fit p-0 bg-white z-40 rounded-lg border">
                <EmojiPickerEmojiPicker setValue={e => console.log(e)} />
            </PopoverContent>
        </Popover>
    );
};
