...
export type FilterOption = {
    name: FilterType | Status | WorkflowStatus | Assignee | Labels | Priority | DueDate;
...
const FilterIcon = ({ type }: { type: FilterType | Status | WorkflowStatus | Assignee | Labels | Priority }) => {
    switch (type) {
        ...
        case FilterType.STARTED_AT:
            return <CalendarPlus className="size-3.5" />;
        case FilterType.COMPLETED_AT:
        ...
        case WorkflowStatus.READY:
            return <Circle className="size-3.5 text-gray-500" />;
        ...
        case WorkflowStatus.BACKING:
            return <CircleDashed className="size-3.5 text-gray-400" />;
        case WorkflowStatus.SUSPENDED:
            return <CircleDashed className="size-3.5 text-orange-400" />;
        case WorkflowStatus.PAUSED:
            return <CircleDashed className="size-3.5 text-yellow-400" />;
...
export const filterViewOptions: FilterOption[][] = [
    ...
    [
        ...
        {
            name: FilterType.STARTED_AT,
            icon: <FilterIcon type={FilterType.STARTED_AT} />,
        },
        {
            name: FilterType.COMPLETED_AT,
            icon: <FilterIcon type={FilterType.COMPLETED_AT} />,
        },
    ],
];
...
export const workflowStatusFilterOptions: FilterOption[] = Object.values(WorkflowStatus).map(status => ({
    name: status,
    icon: <FilterIcon type={status} />,
}));
...
export const filterViewToFilterOptions: Record<FilterType, FilterOption[]> = {
    ...
    [FilterType.WORKFLOW_STATUS]: workflowStatusFilterOptions,
    ...
    [FilterType.STARTED_AT]: dateFilterOptions,
    [FilterType.COMPLETED_AT]: dateFilterOptions,
};
...
const FilterOperatorDropdown = ({
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger className="bg-muted hover:bg-muted/50 px-1.5 py-2 text-muted-foreground hover:text-primary transition shrink-0">
                {operator}
            </DropdownMenuTrigger>
...
const FilterValueCombobox = ({
    ...
    return (
        ...
        >
            <PopoverTrigger
                className="rounded-none px-1.5 py-2 bg-muted/10 hover:bg-muted/50 transition
...
const FilterValueDateCombobox = ({
    ...
    return (
        ...
        >
            <PopoverTrigger
                className="rounded-none px-1.5 py-2 bg-muted hover:bg-muted/50 transition
  text-muted-foreground hover:text-primary shrink-0"
            >
                {filterValues?.[0]}
            </PopoverTrigger>
    ...
    return (
        <div className="flex gap-2">
            {filters
                ...
                .map(filter => (
                    <div key={filter.id} className="flex gap-[0.5px] items-center text-xs">
                        <div className="flex gap-1.5 shrink-0 rounded-l bg-muted px-1.5 py-2 items-center">
                        ...
                        filter.type === FilterType.STARTED_AT ||
                        filter.type === FilterType.COMPLETED_AT ||
                        ...
                        <Button
                            ...
                            className="bg-muted rounded-l-none rounded-r-sm h-8 w-6 text-muted-foreground hover:text-primary hover:bg-muted/50 transition shrink-0"
                        >
                            <X className="size-3" />
                        </Button>
                    </div>
