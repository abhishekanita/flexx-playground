...
export async function syncEmailsForUser(userId: string): Promise<EmailSyncResult> {
    ...
    // Always use a fixed lookback window so interrupted runs don't skip queries.
    // DB dedup handles overlap cheaply. On first run this catches 3 months of history.
    const afterDate = new Date(Date.now() - 3 * 30 * 24 * 60 * 60 * 1000);
    ...
    for (const searchQuery of GMAIL_SEARCH_QUERIES) {
        try {
            ...
            const ops = newMessages.map(msg => ({
                insertOne: { document: toRawEmailDoc(userId, msg, now) },
            }));
            ...
            try {
                ...
                logger.info(`[EmailSync] Query "${searchQuery.id}": ${messages.length} fetched, ${inserted} inserted, ${skipped} duplicates`);
            } catch (err: any) {
                const inserted = err.result?.nInserted || 0;
                result.newEmails += inserted;
                result.errors++;
                logger.error(`[EmailSync] Query "${searchQuery.id}" bulk insert partial failure: ${err.message} (${inserted} inserted)`);
            }
        } catch (err: any) {
    ...
    logger.info(`[EmailSync] Done for user ${userId}: ${result.fetched} fetched, ${result.newEmails} new, ${result.skippedDuplicates} duplicates, ${result.errors} errors`);
    return result;
}

function toRawEmailDoc(userId: string, msg: GmailFullMessage, now: string) {
    ...
    const hasExcel = attachments.some(
        a => a.mimeType === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || a.mimeType === 'application/vnd.ms-excel'
    );
