...
export interface PhonePeTransaction {
    ...
    direction: 'paid' | 'received' | 'wallet_load';
...
export function parsePhonePeStatement(text: string): PhonePeStatement {
    ...
    const txnPattern = /([A-Z][a-z]{2} \d{1,2}, \d{4})\s+(\d{1,2}:\d{2} [AP]M)\s+(Paid to|Received from|Money added to)\s+(.+?)\s+Transaction ID : (\S+)\s+UTR No : (\S+)\s+(?:Paid by|Debited from|Credited to) (XX\d+)\s+(Debit|Credit)\s+INR\s*([\d,.]+)/g;
    ...
    while ((match = txnPattern.exec(text)) !== null) {
        transactions.push({
            date: match[1],
            time: match[2],
            direction: match[3].startsWith('Money') ? 'wallet_load' : match[3].startsWith('Paid') ? 'paid' : 'received',
            payee: match[4].trim(),
            transactionId: match[5],
            utrNo: match[6],
            account: match[7],
            type: match[8] as 'Debit' | 'Credit',
            amount: parseFloat(match[9].replace(/,/g, '')) || 0,
        });
    }
