import { Document, FilterQuery, Model, ObjectId, PipelineStage, UpdateQuery } from 'mongoose';

export abstract class BaseService<T extends Document> {
	protected readonly model: Model<T>;
	protected readonly paginatedResultsLabel: string = 'docs';
	...
	async update(conditions: FilterQuery<T>, update: UpdateQuery<T>): Promise<any> {
		return await this.model.updateOne(conditions, update, { new: true });
	}

	async updateOne(conditions: FilterQuery<T>, update: UpdateQuery<T>): Promise<any> {
	...
	async findByIdAndUpdate(id: ObjectId | string, update: any): Promise<T | null> {
